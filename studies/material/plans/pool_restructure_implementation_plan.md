...
>
> **Status 2026-09-12.** Phases B, D and F were superseded on 2026-07-21 by [bot_extraction_plan.md](bot_extraction_plan.md) (one bot = one variant = one future sub-account; pools re-enter as groups of bots that share a margin model, see D9). Phase A: A2 done (0 replay variants left), A4 done for the six running sleeves (`docs/calibration/`), A1 moot (the orchestrator has been dormant since 2026-06-11), A3 dormant, A5 partly (fact sheet corrected inline), **A6 open** (`strategy_health.KNOWN_SLEEVES` still lacks CHENTO_TRIPLE_V3 / SHORT_SQUEEZE / SQUEEZE_BULL), **A7 open** (`chento_limit_bid` still in the tree). Phase C: S2 answered by `studies/notebooks/validation_audit_2026_09` (nothing clears DSR 0.95), S4 partly by `sizing_style_2026_09` (liquidation walk, no liquidations at shipped sizes) and re-scoped by D9, S5 partly by the R4 decisions (bot 09-06, held 09-09, ETH windows only 09-12); S1, S3, S6 not started. Phase E: SQUEEZE_BULL shipped 2026-09-09 as S-107, OI-flush only (Rule B deferred), plus a no-stop paper twin 09-12. Decisions D8 and D9 added; D1–D7 unchanged since June. The live roadmap is the top section of `BACKLOG.md`. A1 (delete the legacy dispatch path) and A7 (archive `chento_limit_bid`) are now scheduled inside the BACKLOG item "Bot = directory = strategy" (2026-09-12), which retires the orchestrator layer altogether.
>
> **Status 2026-09-13 — A1 and A7 are DONE.** The "Bot = directory = strategy" refactor shipped (BACKLOG steps 1–4). **A1**: `strategies/orchestrator.py`, `bot.py`, `strategies/p300_spec.py`, `backtest_runner.py`, sim mode and the TIMING_ANOMALIES dispatcher were deleted; `strategies/sleeves/` no longer exists and each running strategy lives under its own `bots/<name>/strategy/`. **A7**: `chento_limit_bid` was archived to `studies/material/archive/chento_limit_bid/` along with seven other dormant sleeves — see that directory's `README.md` for the contract (importable for parity studies, nothing live may import it, enforced by `tests/test_archive_is_not_live.py`). **A6** has since closed too — `strategy_health.KNOWN_SLEEVES` now carries `CHENTO_TRIPLE_V3`, `SHORT_SQUEEZE` and `SQUEEZE_BULL` alongside the archived names, which stay in the tuple deliberately because closed trades under those labels are still in the ledger. That leaves only A3 (dormant) and A5 (partly) unresolved in Phase A. Everything below this line is the June plan as written and is **not** being rewritten; read it as the record of how the sequencing was reasoned, and `BACKLOG.md` for the live roadmap.
...
| D9 (added 2026-09-12) | Account placement of no-stop sleeves — R4, the two no-stop paper twins, the EMA/ETH continuous sleeves and any condition-exit sleeve the coming exit study produces: where may a sleeve whose only exit is the clock or a condition hold collateral? | **Isolate the tail, not the sleeve count.** User proposal 2026-09-12: every no-stop sleeve is either paired with something stable or isolated, so it can only destroy its own account (each pool = one sub-account). Recommendation: no new sub-accounts — **Standard is the no-stop / experimental account** (SHORT_SQUEEZE and SQUEEZE_BULL with their no-stop twins, R4's ETH windows, AI_QUANT); Premier stays Chento-only; Continuous keeps EMA/ETH (no-stop, vol-targeted) paired with CARRY and ADX (D8). The gate for every pairing is S4's margin sim on the joint historical path: zero liquidations at 2× the worst observed move (`studies/notebooks/sizing_style_2026_09/` liquidation walk is the method; measured worst cases today are single-digit % of capital for the twins at 0.5× / 1× notional and −15 to −21 % of variant capital for an R4 three-leg cluster at 3× gross). A sleeve that fails the gate inside its pool moves to isolated margin within that sub-account (Binance per-position isolation, as FOMC) before it earns a sub-account of its own. |
...
| S4 | Per-pool worst-case margin sim (extends existing `margin_sim`): FOMC 10× cross-vs-isolated in Calendar; Carry two-wallet/transfer mechanics and leg leverage (C6); crash-gap test of the 50% buffer claim; D9 no-stop pairings (joint-path liquidation walk at 2× the worst observed move) | C3, C6, D9; fact-sheet "structural liquidation insurance" claim | Margin mode per sleeve per pool decided with numbers; buffer claim quantified |
