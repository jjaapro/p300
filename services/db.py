"""services.db — canonical paths to the SQLite databases the bot uses.
...
attribute lookup, not ``from services.db import DASH_DB``) so that
``monkeypatch.setattr("services.db.DASH_DB", ...)`` actually reaches them.
