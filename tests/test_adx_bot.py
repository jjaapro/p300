...
def test_entry_path_raising_still_runs_the_backstop(ledger, monkeypatch):
    ...
    db_path, variant, tid = ledger
    monkeypatch.setattr("botlib.stale_tables", lambda tables=None: {})
    monkeypatch.setattr("bots.adx.strategy.signal.decide", lambda *a, **k: ([object()], {"status": "signal"}))
