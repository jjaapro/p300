...
def test_sim_accepts_isolated_dbs(tmp_path, monkeypatch):
    ...
    so we don't actually loop). Exits 0, and the schema init lands in
    the sim ledger — never in prod.db (review 2026-09-06 finding 3:
    trade_db used to capture PROD_DB at import, so init_db() wrote there
    and the sim ledger had no trades table).
    ...
    assert rc == 0
    con = sqlite3.connect(str(tmp_path / "sim_dash.db"))
    try:
    ...
    assert prod_path.stat().st_size == 0        # the live DB was never touched
