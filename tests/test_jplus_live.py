...
timing across the four R4 variants.

Each test sets a synthetic clock, monkey-patches price_feed and
today_inputs, and asserts the resulting trades-table state.

Twelve more tests covered the two continuous-position handlers, EMA_BTC and
ETH_DAILY, until 2026-09-13. Both sleeves were archived that day — EMA_BTC had
booked zero trades as of the 2026-06-07 audit and ETH_DAILY never traded live —
and their tests went with them. The R4 handlers live at bots/r4/strategy/.

The ``jplus_live`` namespace below survives that cut: it collects the handlers
under one accessor so the per-test calls (``jplus_live.r4_btc_try_fire(...)``)
