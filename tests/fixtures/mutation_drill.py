...
MUTATIONS = [
    ...
    ("r4: bear-regime zero-weight gate removed",
     ...
     "tests/test_golden_r4.py"),
    ("carry: legacy adapter loses the effective-weight override",
     "strategies/sleeves/carry/signal.py",
     '"_effective_weight_pct", sleeve_cfg.get("weight_pct", 0.0))),',
     '"weight_pct", 0.0)),',
     "tests/test_sleeve_adapter_equivalence.py"),
    ("carry: legacy adapter drops the leverage override",
     "strategies/sleeves/carry/signal.py",
    ...
    ("adx: legacy adapter drops the leverage override",
     "strategies/sleeves/adx/signal.py",
     '"leverage": float(sleeve_cfg.get("_effective_leverage", 1.0)),',
     '"leverage": 1.0,',
     "tests/test_sleeve_adapter_equivalence.py"),
    ("adx: legacy adapter loses the params stop_loss_pct",
     "strategies/sleeves/adx/signal.py",
    ...
    ("short_squeeze: legacy adapter conflates count_diag with use_stop",
     "strategies/sleeves/short_squeeze/signal.py",
     '"count_diag": bool(sleeve_cfg.get("count_diag", True)),',
     '"count_diag": bool(sleeve_cfg.get("use_stop", True)),',
     "tests/test_sleeve_adapter_equivalence.py"),
    ("short_squeeze: macro gate removed",
     "strategies/sleeves/short_squeeze/signal.py",
     'if not macro["is_short_macro"]:', 'if False:',
     "tests/test_golden_short_squeeze.py"),
    ("chento: legacy adapter loses the effective-weight override",
